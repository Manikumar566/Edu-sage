...
def _parse_flashcards(raw: str) -> list:
    ...
    for line in lines:
        if line.lower().startswith("q:"):
            ...
            # join continuation lines sensibly
    ...
    if q and a:
        cards.append({"question": q, "answer": a})
    # fallback: if model produced bullet pairs
    if not cards:
...
class QASystem:
    def __init__(self):
        # each chunk: {'id', 'text', 'source', 'page', 'vec'}
        self.chunks: List[Dict] = []
        self.outputs_dir = os.path.join(os.getcwd(), "outputs")
        os.makedirs(self.outputs_dir, exist_ok=True)
        # Auto-clear outputs at start
    ...
    def summarize_all(self) -> str:
        if not self.chunks:
            return "No PDFs indexed yet. Upload in the sidebar."
        # concatenate a reasonable amount of content
